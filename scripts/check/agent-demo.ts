 ...
 *
 *   node --experimental-strip-types scripts/check/agent-demo.ts <base-url> "<query>" [maxPrice]
 *   node --experimental-strip-types scripts/check/agent-demo.ts https://your-store.example.com "warm hat" 40
 *
...
if (!base) {
  console.error(
    'Usage: node --experimental-strip-types scripts/check/agent-demo.ts <base-url> "<query>" [maxPrice]',
  );
  process.exit(1);
}
