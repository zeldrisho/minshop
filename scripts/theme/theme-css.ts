 ...
 * disk plus config/theme.config.json. The THEME environment variable never
 ...
 *   - the `#theme` and `#theme-css` Vite aliases (astro.config.ts,
 *     vitest.config.ts)
...
import { CONFIG_FILE, THEMES_DIR, discoverThemeIds, resolveConfiguredTheme } from "./themes.ts";

/** One stylesheet per theme, so selecting a theme is an alias, not a rewrite. */
export const GENERATED_CSS_DIR = "src/styles/themes";
/** The editor-facing tsconfig fragment; follows config/theme.config.json only. */
export const SHARED_TSCONFIG = "tsconfig.theme.json";

export function themeCssPath(id: string, root = process.cwd()): string {
  return resolve(root, GENERATED_CSS_DIR, `${id}.css`);
}

export function themeTsconfigPath(id: string, root = process.cwd()): string {
  return resolve(root, `tsconfig.theme.${id}.json`);
}

export function themeCss(id: string, root = process.cwd()): string {
 ...
 * Written by scripts/theme/theme-css.ts. One of these exists per theme; the
...
function themeTsconfig(id: string): string {
  return `${JSON.stringify(
    {
      $comment: `GENERATED by scripts/theme/theme-css.ts — selects theme: ${id}. Do not edit or commit. Consumed via astro check --tsconfig.`,
...
function sharedTsconfig(id: string): string {
  return `${JSON.stringify(
    {
      $comment: `GENERATED by scripts/theme/theme-css.ts from ${CONFIG_FILE} — theme: ${id}. Do not edit or commit.`,
      compilerOptions: { paths: { "#theme/*": [`./${THEMES_DIR}/${id}/*`] } },
    },
    null,
    2,
...
function writeIfChanged(file: string, next: string): string {
  try {
 ...
 * a missing directory. Called by astro.config.ts (so a bare `astro dev` on a
...
export function writeThemeArtifacts(root = process.cwd()) {
  ...
  writeIfChanged(
    ...
    `/*
 * GENERATED — do not edit, and do not commit.
 * Written by scripts/theme/theme-css.ts for src/styles/admin.css: keeps every
